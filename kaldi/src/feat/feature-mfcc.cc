...
void MfccComputer::Compute(BaseFloat signal_raw_log_energy,
  ...
  // for (int32 i = 0; i < mel_energies_.Dim(); ++i) {
  //   std::cout << "mel_energies_[" << i << "] = " << mel_energies_(i) << std::endl;
  // }
...
std::cout << "opts_.cepstral_lifter = " << opts_.cepstral_lifter << std::endl;
  if (opts_.cepstral_lifter != 0.0)
    feature->MulElements(lifter_coeffs_);

    // for (int32 i = 0; i < feature->Dim(); ++i) {
...
MfccComputer::MfccComputer(const MfccOptions &opts):
  ...
  // for (int32 i = 0; i < dct_matrix_.NumRows(); ++i) {
  //   for (int32 j = 0; j < dct_matrix_.NumCols(); ++j) {
  //     std::cout << "dct_matrix(" << i << ", " << j << ") = " << dct_matrix_(i, j) << std::endl;
  //   }
  // }
