...
MelBanks::MelBanks(const MelBanksOptions &opts,
  ...
  if (low_freq < 0.0 || low_freq >= nyquist
    ...
    KALDI_ERR << "Bad values in options: low-freq " << low_freq
              ...
              << nyquist;
              // std::cout << "low_freq: " << low_freq << std::endl;
              // std::cout << "high_freq: " << high_freq << std::endl;
  ...
  // std::cout << "vtln_low: " << vtln_low << std::endl;
  // std::cout << "vtln_high: " << vtln_high << std::endl;
  ...
  for (int32 bin = 0; bin < num_bins; bin++) {
    ...
    // for (int32 i = 0; i < bins_[bin].second.Dim(); i++) {
    //   std::cout << "bins_[" << bin << "].second[" << i << "]: " << bins_[bin].second(i) << std::endl;
    // }
...
void MelBanks::Compute(const VectorBase<BaseFloat> &power_spectrum,
  ...
  for (int32 i = 0; i < num_bins; i++) {
    ...
    SubVector<BaseFloat> temp = power_spectrum.Range(offset, v.Dim());
